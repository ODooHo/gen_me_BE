...
public class LoginController {
    ...
    @PostMapping()
    public ResponseDto<?> startGame(@RequestBody StartDto startDto){
        if(loginService.checkUser(startDto)) return ResponseDto.error("닉네임이 중복됩니다.");
        loginService.loginUser(startDto);
        if(startDto.getMaxCount() != 0){
            return ResponseDto.success(gameService.createGame(startDto));
        }
        else if(startDto.getInvitationCode() != null){
            return ResponseDto.success(gameService.joinGame(startDto));
        }
        return ResponseDto.success("방을 생성하거나, 입장해야합니다.");
    }
}
